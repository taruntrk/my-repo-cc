...
def execute_pattern_08():
    print("="*60)
    print("STARTING GENDER & RELATIONSHIP LEAKAGE EXTRACTION (PATTERN 08)")
    print("Targeted: All Claims with Deductions >= 10,000 INR (No Row Limits)")
    print("="*60)
    ...
    summary_filepath = os.path.join(data_dir, f"new_08a_gender_relation_summary_{timestamp}.csv")
    claims_filepath = os.path.join(data_dir, f"new_08b_gender_relation_claims_{timestamp}.csv")
    
    try:
        print(f"[{datetime.datetime.now()}] Connecting to SSH ({SSH_HOST})...")
        client.connect(SSH_HOST, port=SSH_PORT, username=SSH_USER, password=SSH_PASS)
        
        # ----------------- PART A: SUMMARY GROUPING -----------------
        print(f"[{datetime.datetime.now()}] Running 08a: Gender & Relationship Grouped Leakage Summary...")
        query_a = """
            SELECT
        ...
        with open(summary_filepath, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['gender', 'relationship', 'total_claims', 'total_claimed_cr', 'total_approved_cr', 'total_deducted_cr', 'deduction_pct', 'hospitals_involved'])
        ...
        # ----------------- PART B: SINGLE-STREAM CLAIMS EXTRACTION -----------------
        print(f"[{datetime.datetime.now()}] Running 08b: Extracting detailed metadata for claims with >= 10k deductions...")
        query_b = """
            SELECT
                ci.CI_SEX                                                                   AS gender,
        ...
        with open(claims_filepath, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'gender', 'relationship', 'hospital_id', 'hospital_name', 'city', 'state',
                'hosp_type', 'nabh_status', 'claim_id', 'service_number', 'card_number',
                'beneficiary_name', 'rank', 'service_type', 'patient_name', 'age',
            ...
            for line in stdout_b:
                ...
                if line:
                    ...
                    if total_claims_saved % 50000 == 0:
                        print(f"[{datetime.datetime.now()}] Progress: Extracted {total_claims_saved:,} demographic claims...")
        ...
        print(f"CRITICAL ERROR in Pattern 08: {e}")
        traceback.print_exc()
    finally:
        client.close()
