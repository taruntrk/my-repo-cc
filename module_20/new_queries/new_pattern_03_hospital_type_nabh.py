...
def execute_pattern_03():
    print("="*60)
    print("STARTING HOSPITAL TYPE & NABH LEAKAGE EXTRACTION (PATTERN 03)")
    print("Targeted: All Claims for Types M, N, H, 0 with Deductions >= 10,000 INR")
    print("="*60)
    ...
    summary_filepath = os.path.join(data_dir, f"new_03a_hospital_type_nabh_summary_{timestamp}.csv")
    claims_filepath = os.path.join(data_dir, f"new_03b_type_all_deduction_claims_{timestamp}.csv")
    ...
    try:
        ...
        # ----------------- PART A: SUMMARY -----------------
        print(f"[{datetime.datetime.now()}] Running 03a: Hospital Type & NABH Summary...")
        query_a = """
            SELECT
        ...
        with open(summary_filepath, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['hosp_type_code', 'hosp_type_desc', 'nabh_status', 'num_hospitals', 'total_claims', 'total_claimed_cr', 'total_approved_cr', 'total_deducted_cr', 'deduction_pct'])
        ...
        # ----------------- PART B: TARGETED TYPE EXTRACTION -----------------
        print(f"[{datetime.datetime.now()}] Running 03b: Streaming type-specific claims with >= 10k deductions...")
        query_b = """
            SELECT
            ...
            WHERE ci.CI_ADMISSION_DATE >= '2021-04-01'
              AND om.OM_HOSP_TYPE IN ('M', 'N', 'H', '0')
              AND (cs.CS_NET_CLAIM_AMT - cs.CS_UTI_APP_AMT) >= 10000
        ...
        with open(claims_filepath, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'hosp_type_code', 'hosp_type_desc', 'nabh_status', 'hospital_id', 'hospital_name',
                'city', 'state', 'claim_id', 'service_number', 'card_number', 'beneficiary_name',
                'rank', 'service_type', 'patient_name', 'age', 'gender', 'relationship',
                'admission_date', 'discharge_date', 'stay_days', 'ailment', 'treating_doctor',
                'bill_number', 'claimed_amount', 'approved_amount', 'deducted_amount', 'deduction_pct',
            ...
            for line in stdout_b:
                ...
                if line:
                    ...
                    if total_claims_saved % 50000 == 0:
                        print(f"[{datetime.datetime.now()}] Progress: Extracted {total_claims_saved:,} hospital-type claims...")
        ...
        print(f"CRITICAL ERROR in Pattern 03: {e}")
        traceback.print_exc()
    finally:
        client.close()
