...
def execute_pattern_02():
    print("="*60)
    print("STARTING ANNUAL EXPENDITURE TREND EXTRACTION (PATTERN 02)")
    print("Targeted: All Claims with Deductions >= 20,000 INR (No Row Limits)")
    print("="*60)
    ...
    summary_filepath = os.path.join(data_dir, f"new_02a_annual_expenditure_trend_{timestamp}.csv")
    claims_filepath = os.path.join(data_dir, f"new_02b_annual_top_deduction_claims_{timestamp}.csv")
    ...
    try:
        ...
        # ----------------- PART A: SUMMARY -----------------
        print(f"[{datetime.datetime.now()}] Running 02a: Annual Expenditure Trend...")
        query_a = """
            SELECT
        ...
        with open(summary_filepath, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['fiscal_year', 'total_claims', 'total_claimed_cr', 'total_approved_cr', 'total_deducted_cr', 'deduction_pct', 'hospitals_active'])
        ...
        # ----------------- PART B: ANNUAL DETAILED EXTRACTION -----------------
        print(f"[{datetime.datetime.now()}] Running 02b: Streaming annual claims with >= 20k deductions...")
        query_b = """
            SELECT
            ...
            WHERE ci.CI_ADMISSION_DATE >= '2021-04-01'
              AND (cs.CS_NET_CLAIM_AMT - cs.CS_UTI_APP_AMT) >= 20000
        ...
        with open(claims_filepath, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'admission_year', 'hospital_id', 'hospital_name', 'city', 'state', 'hosp_type',
                'nabh_status', 'claim_id', 'service_number', 'card_number', 'beneficiary_name', 'rank_name',
            ...
            for line in stdout_b:
                ...
                if line:
                    ...
                    if total_claims_saved % 50000 == 0:
                        print(f"[{datetime.datetime.now()}] Progress: Extracted {total_claims_saved:,} annual claims...")
        ...
        print(f"CRITICAL ERROR in Pattern 02: {e}")
        traceback.print_exc()
    finally:
        client.close()
