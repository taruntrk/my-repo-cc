"""
Module 20 — Pattern 05: Regional Deduction Breakdown
05a: All regions ranked by deduction (city+state names via cghs_region_master)
05b: ALL individual claims from top 10 highest-deduction regions (2021-2026)
"""
...
QUERY_B = """
    ...
    JOIN  settlement_stat ss    ON ci.CI_CR_OFFICE_ID  = ss.SS_OFFICE_ID
                                AND ss.SS_FY_YEAR BETWEEN 2021 AND 2025
    ...
    WHERE ci.CI_CR_OFFICE_ID IS NOT NULL
        AND YEAR(ci.CI_ADMISSION_DATE) BETWEEN 2021 AND 2026
        AND ss.SS_REGION_ID IN ({ids})
        AND COALESCE(cs.CS_NET_CLAIM_AMT, 0) > 0
        AND (COALESCE(cs.CS_NET_CLAIM_AMT, 0) - COALESCE(cs.CS_UTI_APP_AMT, 0)) > 0
    ORDER BY region_name, deducted_amount DESC
    LIMIT 5000;
"""
...
def main():
    ...
    try:
        ...
        # 05a
        rows_a = run_query(QUERY_A, '05a_regional_deduction_breakdown')
        if len(rows_a) > 1:
            path = os.path.join(data_dir, f'05a_regional_deduction_breakdown_{ts}.csv')
            with open(path, 'w', newline='', encoding='utf-8') as f:
                csv.writer(f).writerows(rows_a)
            print(f"  ✅ Saved → {os.path.basename(path)}")

        # top 10 regions
        top_rows = run_query(TOP10_REGIONS_QUERY, 'top10_regions_lookup')
        ids = [r[0] for r in top_rows[1:] if r]
        if not ids:
            print("  ⚠ No region IDs found"); return
        ids_str = ', '.join(str(r) for r in ids)
        print(f"  Top regions: {ids}")

        # 05b
        rows_b = run_query(QUERY_B.format(ids=ids_str), '05b_regional_all_claims')
        if len(rows_b) > 1:
            path = os.path.join(data_dir, f'05b_regional_all_claims_{ts}.csv')
            with open(path, 'w', newline='', encoding='utf-8') as f:
                csv.writer(f).writerows(rows_b)
            print(f"  ✅ Saved → {os.path.basename(path)}")
        else:
            print("  ⚠ No data for 05b")
