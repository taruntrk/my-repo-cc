"""
Module 20 — Pattern 07: High Deduction Hospital Claims
07a: Top 50 hospitals by total deduction amount (settlement_stat)
07b: ALL individual claims for those top 50 hospitals (2021-2026)
"""
...
QUERY_A = """
    ...
    GROUP BY ss.SS_OFFICE_ID, om.OM_OFFICE_NAME, crm.CRM_CITY_NAME,
             sm.SM_STATE_NAME, om.OM_HOSP_TYPE, om.OM_NABH
...
def main():
    ...
    try:
        ...
        # 07a
        rows_a = run_query(QUERY_A, '07a_high_deduction_hospitals')
        if len(rows_a) > 1:
            path = os.path.join(data_dir, f'07a_high_deduction_hospitals_{ts}.csv')
        ...
        if not ids:
            print("  ⚠ No hospital IDs found"); return
        ids_str = ', '.join(str(h) for h in ids)
        print(f"  Top 50: {ids[:3]}... ({len(ids)} total)")

        # 07b
        rows_b = run_query(QUERY_B.format(ids=ids_str), '07b_high_deduction_all_claims')
        if len(rows_b) > 1:
            path = os.path.join(data_dir, f'07b_high_deduction_all_claims_{ts}.csv')
            with open(path, 'w', newline='', encoding='utf-8') as f:
                csv.writer(f).writerows(rows_b)
            print(f"  ✅ Saved → {os.path.basename(path)}")
        else:
            print("  ⚠ No data for 07b")
